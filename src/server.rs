...
fn parse_data_keys(s: &str) -> Vec<serde_json::Value> {
    ...
    let push = |tok: &str, fields: &mut Vec<serde_json::Value>| {
        let t = tok.trim();
        if t.is_empty() {
            return;
        }
        if let Some(name) = t.strip_suffix("[]") {
            ...
            let item_keys: Vec<&str> = inner
                .split(',')
                .map(str::trim)
                .filter(|s| !s.is_empty())
                .collect();
            fields.push(
                serde_json::json!({"name": name, "type": "array<object>", "item_keys": item_keys}),
            );
        } else {
...
fn build_labels(
    input: &ChartInput,
    kind: zavora_docx::ChartKind,
) -> Option<zavora_docx::DataLabels> {
    ...
    if let Some(v) = input.label_show_value {
        l.show_value = v;
    }
    if let Some(v) = input.label_show_category {
        l.show_category = v;
    }
    if let Some(v) = input.label_show_percent {
        l.show_percent = v;
    }
    if input.label_color.is_some() {
        l.color = input.label_color.clone();
    }
    Some(l)
}
...
#[derive(Debug, Deserialize, schemars::JsonSchema)]
pub struct ReadParasInput {
    pub document_handle: String,
    pub offset: Option<usize>,
    pub limit: Option<usize>,
}
#[derive(Debug, Deserialize, schemars::JsonSchema)]
pub struct ReadParaInput {
    pub document_handle: String,
    pub index: usize,
}
#[derive(Debug, Deserialize, schemars::JsonSchema)]
...
#[derive(Debug, Deserialize, schemars::JsonSchema)]
pub struct ParaRunsInput {
    pub document_handle: String,
    pub index: usize,
    pub runs: Vec<RunSpec>,
}
#[derive(Debug, Deserialize, schemars::JsonSchema)]
pub struct RunFormatInput {
    pub document_handle: String,
...
#[derive(Debug, Deserialize, schemars::JsonSchema)]
pub struct CommentRangeInput {
    pub document_handle: String,
    pub paragraph_index: usize,
    pub comment_id: u32,
    pub commented_text: String,
}
...
#[derive(Debug, Deserialize, schemars::JsonSchema)]
pub struct ResolveTrackedInput {
    pub document_handle: String,
...
pub struct ProtectInput {
    pub document_handle: String,
    /// "readonly", "forms", "comments", "trackedChanges"
...
pub struct DropCapInput {
    pub document_handle: String,
    pub paragraph_index: usize,
    /// Number of lines to span (2-4)
    pub lines: u32,
}

#[derive(Debug, Deserialize, schemars::JsonSchema)]
pub struct TextEffectInput {
    pub document_handle: String,
    pub paragraph_index: usize,
    pub text: String,
...
impl DocxServer {
    ...
    #[tool(
        description = "List business document templates for create_document. Returns 'templates' (each: format id, description, 'data_keys' human summary, and structured 'data_fields' with name/type/item_keys) and 'style_params' (universal styling keys every template accepts: name, type, default, note)."
    )]
    pub async fn list_templates(&self) -> String {
        let templates: Vec<serde_json::Value> = engine::template_catalog()
            .into_iter()
            .map(|(format, description, data_keys)| {
                serde_json::json!({
                    "format": format,
            ...
            .collect();
        let style_params: Vec<serde_json::Value> = engine::style_params()
            .into_iter()
            .map(|(name, ty, default, note)| {
                serde_json::json!({
    ...
    pub async fn create_novel(&self, Parameters(input): Parameters<NovelInput>) -> String {
        ...
        let cfg = engine::NovelConfig {
            ...
            author: input.author,
            trim: (
                input.trim_width.unwrap_or(5.25),
                input.trim_height.unwrap_or(8.0),
            ),
    ...
    #[tool(
        description = "Insert a paragraph with optional style and page break. Styles: Heading1, Heading2, Heading3, BodyText, BodyTextIndent, ChapterNum, TitlePage, Subtitle, Author, Copyright"
    )]
            ...
            |doc: &mut zavora_docx::Document| {
                ...
                match style {
                    ...
                    Some("TitlePage") => {
                        para = para
                            .style("Title")
                            .alignment(zavora_docx::Alignment::Center);
                        para.add_run(&input.text);
                    }
                    Some("Subtitle") => {
                        para = para
    ...
    #[tool(
        description = "Insert a syntax-highlighted code block with gray background (Courier New 9pt). Supports 'rust' highlighting."
    )]
    pub async fn insert_code_block(&self, Parameters(input): Parameters<CodeBlockInput>) -> String {
        with_doc!(
            ...
            input.document_handle,
            |doc: &mut zavora_docx::Document| {
                let lines = engine::insert_code_block(
                    doc,
                    input.index,
                    &input.code,
    ...
    #[tool(
        description = "Insert a callout box with colored background and border (tip=green, warning=orange, note=blue)"
    )]
                ...
                match doc.table_mut(ti) {
                    Some(mut table) => match table.cell(input.row, input.col) {
                        Some(mut cell) => {
                            ...
                            serde_json::json!({"set": true, "table": ti, "row": input.row, "col": input.col}).to_string()
                        }
    ...
    pub async fn add_image(&self, Parameters(input): Parameters<ImageInput>) -> String {
        with_doc!(
            ...
            |doc: &mut zavora_docx::Document| {
                ...
                let has_feature = input.rotation.is_some()
                    || input.flip_h.is_some()
                    || input.flip_v.is_some()
                    || input.crop.is_some()
                    || input.border_color.is_some()
                    || input.shadow_color.is_some()
                ...
                if has_feature {
                    let props = zavora_docx::PicProps {
                        ...
                        border: input
                            .border_color
                            .map(|c| (c, (input.border_width.unwrap_or(1.0) * 12700.0) as i64)),
    ...
    #[tool(
        description = "Add a full-bleed page background image anchored to the page at `index` (place a page-break paragraph there first so it covers that page). Text added after flows on top. Use for children's book spreads and full-page interior plates. Returns the anchor index."
    )]
    pub async fn add_page_background(
        &self,
        Parameters(input): Parameters<PageBackgroundInput>,
    ) -> String {
        with_doc!(
            self.store,
            input.document_handle,
    ...
    #[tool(
        description = "Set document-level settings in settings.xml: default tab stop (inches), mirror margins, track changes, open zoom percent, proofing language (e.g. en-US), update fields on open, and auto hyphenation. Only provided fields are changed; existing settings are preserved."
    )]
    pub async fn set_document_settings(
        &self,
        Parameters(input): Parameters<DocumentSettingsInput>,
    ) -> String {
        with_doc!(
            self.store,
            input.document_handle,
    ...
    pub async fn add_content_control(
        &self,
        Parameters(input): Parameters<ContentControlInput>,
    ) -> String {
        with_doc!(
            self.store,
            input.document_handle,
    ...
    #[tool(
        description = "Add a rectangular text box (with a border) containing the given lines of text. Width/height default to 3x1.5 inches."
    )]
    ...
    #[tool(
        description = "Add a preset shape (DrawingML). geometry is a preset name: rect, roundRect, ellipse, triangle, diamond, rightArrow, leftArrow, star5, hexagon, etc. Optional solid fill_color (hex). Width/height default to 2x2 inches."
    )]
        ...
        with_doc!(
            ...
            |doc: &mut zavora_docx::Document| {
                let kind = match input.kind.as_str() {
                    ...
                    other => {
                        return serde_json::json!({"error": format!("unknown chart kind: {other}")})
                            .to_string()
                    }
                };
    ...
    #[tool(
        description = "Insert a scene break (for novels). style: 'asterisks', 'diamond', or 'blank'"
    )]
    pub async fn insert_scene_break(
        &self,
        Parameters(input): Parameters<SceneBreakInput>,
    ) -> String {
        with_doc!(
            self.store,
            input.document_handle,
            |doc: &mut zavora_docx::Document| {
                engine::insert_scene_break(
                    doc,
                    input.index,
                    input.style.as_deref().unwrap_or("asterisks"),
                );
                serde_json::json!({"inserted": true}).to_string()
            }
        )
    ...
    #[tool(
        description = "Insert a best-seller chapter opening: a drop-cap initial letter with the first few words in small caps, then the body text. Use for the FIRST paragraph of each chapter (after the Heading1 chapter title). Pass the whole first paragraph as text."
    )]
    pub async fn insert_chapter_opening(
        &self,
        Parameters(input): Parameters<ChapterOpeningInput>,
    ) -> String {
        with_doc!(
            self.store,
            input.document_handle,
            |doc: &mut zavora_docx::Document| {
                engine::insert_chapter_opening(
                    doc,
                    input.index,
                    &input.text,
                    input.font.as_deref().unwrap_or("Garamond"),
                );
    ...
    #[tool(
        description = "Export document as Markdown. Headings become #/##/###, paragraphs become text blocks."
    )]
    ...
    #[tool(
        description = "Export document as an editable HTML fragment. Each block carries data-p=\"{index}\" mapping to the paragraph index for update_paragraph_text. Use for an in-place editing view."
    )]
    ...
    #[tool(
        description = "Page layout for a paginated view: page width/height and margins (CSS px @96dpi) plus rendered header/footer HTML. Pair with editable_html (body) to render letter-sized pages with headers/footers."
    )]
    ...
    #[tool(
        description = "Engine-authoritative layout: the positioned page frames (glyph runs, lines, rects, images) as laid out by the SAME engine that produces the PDF — true WYSIWYG. Returns {pages:[{page_number,width,height,elements:[...]}]} in typographic points. Render these directly instead of measuring in the browser."
    )]
    ...
    #[tool(
        description = "Raw bytes of an embedded image by its embed/relationship id (the embed_id carried by layout_frames Image elements). Returns {content_type, base64}. Used to display images in the WYSIWYG renderer."
    )]
    ...
    #[tool(
        description = "Read paragraphs with pagination. Returns text and index for each paragraph."
    )]
    ...
    pub async fn read_paragraph(&self, Parameters(input): Parameters<ReadParaInput>) -> String {
        with_doc!(
            ...
            |doc: &mut zavora_docx::Document| {
                let paras = doc.paragraphs();
                if input.index >= paras.len() {
                    return serde_json::json!({"error": "INDEX_OUT_OF_BOUNDS"}).to_string();
                }
    ...
    pub async fn read_table(&self, Parameters(input): Parameters<ReadTableInput>) -> String {
        with_doc!(
            ...
            |doc: &mut zavora_docx::Document| {
                ...
                let ti = input.table_index.unwrap_or(0);
                if ti >= tables.len() {
                    return serde_json::json!({"error": "TABLE_NOT_FOUND"}).to_string();
                }
                let table = &tables[ti];
                ...
                for r in 0..table.row_count() {
                    if let Some(row) = table.row(r) {
                        let cells: Vec<String> = (0..row.cell_count())
                            .filter_map(|c| row.cell(c).map(|cell| cell.text()))
                            .collect();
                        rows.push(cells);
                    }
                }
    ...
    #[tool(
        description = "Search text across all paragraphs. Returns matching paragraph indices and text."
    )]
        ...
        with_doc!(
            ...
            |doc: &mut zavora_docx::Document| {
                ...
                let results: Vec<serde_json::Value> = doc
                    ...
                    .iter()
                    .enumerate()
                    .filter(|(_, p)| p.text().to_lowercase().contains(&query_lower))
                    .map(|(i, p)| serde_json::json!({"index": i, "text": p.text()}))
                    .collect();
    ...
    #[tool(
        description = "Find and replace text across all paragraphs. Returns count of replacements made."
    )]
    ...
    #[tool(
        description = "Replace a paragraph's content with formatted runs, preserving its style. Each run: {text, bold?, italic?, underline?}. Use for in-place edits that keep inline formatting."
    )]
    pub async fn set_paragraph_runs(&self, Parameters(input): Parameters<ParaRunsInput>) -> String {
        with_doc!(
            ...
            input.document_handle,
            |doc: &mut zavora_docx::Document| {
                if input.index >= doc.paragraph_count() {
                    return serde_json::json!({"error": "INDEX_OUT_OF_BOUNDS"}).to_string();
                }
                ...
                let style = doc
                    .paragraphs()
                    .get(input.index)
                    .and_then(|p| p.style_id().map(String::from));
                ...
                for r in &input.runs {
                    ...
                    if r.bold.unwrap_or(false) {
                        run = run.bold(true);
                    }
                    if r.italic.unwrap_or(false) {
                        run = run.italic(true);
                    }
                    if r.underline.unwrap_or(false) {
                ...
                serde_json::json!({"updated": true, "index": input.index, "runs": input.runs.len()})
                    .to_string()
            }
        )
    }
    ...
    pub async fn update_paragraph_text(
        &self,
        Parameters(input): Parameters<UpdateParaInput>,
    ) -> String {
        with_doc!(
            self.store,
            input.document_handle,
    ...
    #[tool(
        description = "Add a formatted text run to a paragraph. Supports bold, italic, underline, font, size, color."
    )]
            ...
            |doc: &mut zavora_docx::Document| {
                match doc.paragraph_mut(input.paragraph_index) {
                    Some(mut para) => {
                        ...
                        if input.italic.unwrap_or(false) {
                            run = run.italic(true);
                        }
                        if input.underline.unwrap_or(false) {
                            run = run.underline(true);
                        }
                        ...
                        serde_json::json!({"added": true, "paragraph_index": input.paragraph_index})
                            .to_string()
                    }
    ...
    #[tool(
        description = "Add a Word field to a paragraph. instruction is the field code, e.g. 'DATE \\\\@ \"yyyy-MM-dd\"', 'REF bookmarkName', 'SEQ Figure \\\\* ARABIC', 'STYLEREF \"Heading 1\"', 'PAGE', 'NUMPAGES'. Optional cached result text. Word recomputes fields on open when update-fields is enabled (see set_document_settings update_fields)."
    )]
    ...
    #[tool(
        description = "Change an existing paragraph's named style by index (e.g. Heading1, Heading2, Heading3, Title, Subtitle, Normal). Preserves the paragraph's text."
    )]
    pub async fn set_paragraph_style(
        &self,
        Parameters(input): Parameters<ParaStyleInput>,
    ) -> String {
        with_doc!(
            self.store,
            input.document_handle,
            |doc: &mut zavora_docx::Document| {
                match doc.paragraph_mut(input.index) {
                    Some(para) => {
                        para.style(&input.style);
                        serde_json::json!({"styled": true, "index": input.index}).to_string()
                    }
                    None => serde_json::json!({"error": "INDEX_OUT_OF_BOUNDS"}).to_string(),
                }
            }
    ...
    pub async fn set_paragraph_format(
        &self,
        Parameters(input): Parameters<ParaFormatInput>,
    ) -> String {
        with_doc!(
            self.store,
            input.document_handle,
            |doc: &mut zavora_docx::Document| {
                match doc.paragraph_mut(input.index) {
                    Some(mut para) => {
                        ...
                        if let Some(sb) = input.space_before {
                            para = para.space_before(zavora_docx::Length::pt(sb));
                        }
                        if let Some(sa) = input.space_after {
                            para = para.space_after(zavora_docx::Length::pt(sa));
                        }
                        if let Some(ls) = input.line_spacing {
                            para.line_spacing_multiple(ls);
                        }
        ...
        with_doc!(
            ...
            |doc: &mut zavora_docx::Document| {
                ...
                for item in &input.items {
                    if is_numbered {
                        doc.add_numbered_list_item(item, 0);
                    } else {
                        doc.add_bullet_list_item(item, 0);
                    }
                }
    ...
    #[tool(
        description = "Insert a table with headers and pre-populated row data. This is the recommended way to create tables with content."
    )]
    pub async fn insert_table_with_data(
        &self,
        Parameters(input): Parameters<TableWithDataInput>,
    ) -> String {
        with_doc!(
            self.store,
            input.document_handle,
    ...
    #[tool(
        description = "Merge table cells. direction: 'horizontal' (grid_span across columns) or 'vertical' (v_merge across rows). span = number of cells to merge."
    )]
    pub async fn merge_table_cells(
        &self,
        Parameters(input): Parameters<MergeCellsInput>,
    ) -> String {
        with_doc!(
            self.store,
            input.document_handle,
                ...
                match doc.table_mut(ti) {
                    Some(mut table) => {
                        ...
                        for (c, text) in input.cells.iter().enumerate() {
                            if !text.is_empty() {
                                if let Some(mut cell) = row.cell(c) {
                                    cell.set_text(text);
                                }
                            }
                        }
    ...
    #[tool(
        description = "Set page layout: size, orientation, margins, columns, gutter. All measurements in inches."
    )]
        ...
        with_doc!(
            ...
            |doc: &mut zavora_docx::Document| {
                if let (Some(w), Some(h)) = (input.width, input.height) {
                    doc.set_page_size(
                        zavora_docx::Length::inches(w),
                        zavora_docx::Length::inches(h),
                    );
                }
                ...
                if let Some(m) = &input.margins {
                    if m.len() == 4 {
                        doc.set_margins(
                            zavora_docx::Length::inches(m[0]),
                            zavora_docx::Length::inches(m[1]),
                            zavora_docx::Length::inches(m[2]),
                            zavora_docx::Length::inches(m[3]),
                        );
                    }
                }
    ...
    pub async fn set_metadata(&self, Parameters(input): Parameters<SetMetadataInput>) -> String {
        with_doc!(
            ...
            |doc: &mut zavora_docx::Document| {
                ...
                if let Some(k) = &input.keywords {
                    doc.set_keywords(k);
                }
                if let Some(c) = &input.company {
                    doc.set_company(c);
                }
                if let Some(a) = &input.application {
                    doc.set_application(a);
                }
                serde_json::json!({"set": true}).to_string()
            }
        )
    ...
    #[tool(
        description = "Embed a font file (.ttf/.otf) in the document so it travels with the file. Provide the font family name and a path to the font file. Stored obfuscated per the OOXML embedded-font scheme."
    )]
    ...
    #[tool(
        description = "Add a reusable building block (Quick Part) by name with plain-text content, stored in the glossary document. Available for insertion in Word's Quick Parts gallery."
    )]
    async fn add_building_block(
        &self,
        Parameters(input): Parameters<BuildingBlockInput>,
    ) -> String {
        with_doc!(
            self.store,
            input.document_handle,
    ...
    #[tool(
        description = "Attach a custom XML data part (arbitrary XML) to the document, with a datastore itemProps part for content-control data binding."
    )]
    ...
    #[tool(
        description = "Merge another document into this one. Appends all content with an optional section break between them."
    )]
    pub async fn merge_documents(
        &self,
        Parameters(input): Parameters<MergeDocumentsInput>,
    ) -> String {
    ...
    #[tool(
        description = "Get document outline (heading structure). Returns nested heading tree with levels and text."
    )]
    pub async fn document_outline(&self, Parameters(input): Parameters<HandleInput>) -> String {
        with_doc!(
            ...
            |doc: &mut zavora_docx::Document| {
                ...
                let items: Vec<serde_json::Value> = outline
                    .iter()
                    .map(|node| serde_json::json!({"level": node.level, "text": node.text}))
                    .collect();
    ...
    #[tool(
        description = "Find and replace using regex pattern. Supports capture groups ($1, $2) in replacement."
    )]
    ...
    pub async fn format_table(&self, Parameters(input): Parameters<FormatTableInput>) -> String {
        with_doc!(
            ...
            |doc: &mut zavora_docx::Document| {
                ...
                match doc.table_mut(ti) {
                    Some(mut table) => {
                        if let Some(pct) = input.width_pct {
                        ...
                        if let Some(m) = &input.cell_margins {
                            if m.len() == 4 {
                                table.cell_margins(
                                    zavora_docx::Length::pt(m[0]),
                                    zavora_docx::Length::pt(m[1]),
                                    zavora_docx::Length::pt(m[2]),
                                    zavora_docx::Length::pt(m[3]),
                                );
                            }
                        }
    ...
    pub async fn format_table_cell(
        &self,
        Parameters(input): Parameters<FormatCellInput>,
    ) -> String {
        with_doc!(
            self.store,
            input.document_handle,
                    ...
                    Some(mut table) => match table.cell(input.row, input.col) {
                        Some(mut cell) => {
                            ...
                            if let Some(w) = input.width {
                                cell = cell.width(zavora_docx::Length::inches(w));
                            }
                            if input.no_wrap.unwrap_or(false) {
                                cell.no_wrap();
                            }
                            serde_json::json!({"formatted": true}).to_string()
                        }
    ...
    #[tool(
        description = "Insert a section break at the given index. Allows changing page layout for subsequent content."
    )]
    pub async fn add_section_break(
        &self,
        Parameters(input): Parameters<SectionBreakInput>,
    ) -> String {
        with_doc!(
            self.store,
            input.document_handle,
            |doc: &mut zavora_docx::Document| {
                ...
                if let Some(o) = &input.orientation {
                    match o.as_str() {
                        "landscape" => {
                            para = para.section_landscape();
                        }
                        "portrait" => {
                            para = para.section_portrait();
                        }
                        _ => {}
                    }
                }
    ...
    #[tool(
        description = "Audit document for accessibility issues: missing alt text, empty headings, skipped heading levels, low contrast."
    )]
    ...
    #[tool(
        description = "Add a footnote reference (superscript number) to a paragraph. Use the ID from add_footnote."
    )]
    pub async fn add_footnote_ref(
        &self,
        Parameters(input): Parameters<FootnoteRefInput>,
    ) -> String {
        with_doc!(
            self.store,
            input.document_handle,
    ...
    #[tool(
        description = "Add a clickable hyperlink to a paragraph. Creates blue underlined text linking to the URL."
    )]
    ...
    #[tool(
        description = "Insert a cross-reference (REF field) to a bookmark by name in the given paragraph. Shows the bookmarked text and updates when fields refresh. Create the bookmark first with add_bookmark."
    )]
    ...
    #[tool(
        description = "Add a comment to the document. Use add_comment_range to mark which text the comment applies to."
    )]
    ...
    #[tool(
        description = "Mark text in a paragraph as commented. First call add_comment to create the comment, then use this to mark the range."
    )]
    pub async fn add_comment_range(
        &self,
        Parameters(input): Parameters<CommentRangeInput>,
    ) -> String {
        with_doc!(
            self.store,
            input.document_handle,
    ...
    #[tool(
        description = "Add a threaded reply to an existing comment. Provide a new unique id, the parent_id of the comment being replied to, author, and text."
    )]
    pub async fn reply_to_comment(
        &self,
        Parameters(input): Parameters<CommentReplyInput>,
    ) -> String {
        with_doc!(
            self.store,
            input.document_handle,
    ...
    pub async fn resolve_comment(
        &self,
        Parameters(input): Parameters<ResolveCommentInput>,
    ) -> String {
        with_doc!(
            self.store,
            input.document_handle,
    ...
    #[tool(
        description = "Set a diagonal text watermark on every page (e.g. 'DRAFT', 'CONFIDENTIAL')."
    )]
    ...
    pub async fn add_tracked_insert(
        &self,
        Parameters(input): Parameters<TrackedInsertInput>,
    ) -> String {
        with_doc!(
            self.store,
            input.document_handle,
    ...
    pub async fn add_tracked_delete(
        &self,
        Parameters(input): Parameters<TrackedDeleteInput>,
    ) -> String {
        with_doc!(
            self.store,
            input.document_handle,
    ...
    #[tool(
        description = "List all tracked changes (revisions) in the document. Returns each change's id, author, and kind (insertion/deletion) so a reviewer can accept or reject them."
    )]
    ...
    #[tool(
        description = "Accept or reject tracked changes (revisions). Accept makes insertions permanent and removes deletions; reject removes insertions and restores deletions. Resolves a specific change_id, or all changes if omitted."
    )]
    pub async fn resolve_tracked_changes(
        &self,
        Parameters(input): Parameters<ResolveTrackedInput>,
    ) -> String {
        with_doc!(
            self.store,
            input.document_handle,
            |doc: &mut zavora_docx::Document| {
                ...
                let n = if input.accept {
                    doc.accept_tracked_changes(id)
                } else {
                    doc.reject_tracked_changes(id)
                };
    ...
    pub async fn add_form_text_field(
        &self,
        Parameters(input): Parameters<TextFieldInput>,
    ) -> String {
        with_doc!(
            self.store,
            input.document_handle,
            |doc: &mut zavora_docx::Document| {
                match doc.paragraph_mut(input.paragraph_index) {
                    Some(mut para) => {
                        ...
                        para.add_text_field(
                            &input.name,
                            input.default_value.as_deref().unwrap_or(""),
                        );
                        serde_json::json!({"added": true}).to_string()
                    }
                    None => serde_json::json!({"error": "INDEX_OUT_OF_BOUNDS"}).to_string(),
    ...
    #[tool(
        description = "Protect the document. Types: 'readonly', 'forms' (only form fields editable), 'comments' (only comments allowed), 'trackedChanges'."
    )]
    ...
    #[tool(
        description = "Apply a drop cap to a paragraph (large first letter spanning multiple lines, for chapter openers)."
    )]
    ...
    #[tool(
        description = "Add text with a w14 effect (shadow, glow, outline, or reflection). These are Word 2010+ effects."
    )]
                ...
                match doc.paragraph_mut(input.paragraph_index) {
                    Some(mut para) => {
                        ...
                        match input.effect.as_str() {
                            "shadow" => {
                                run.shadow(size, size * 0.7, color);
                            }
                            "glow" => {
                                run.glow(size, color);
                            }
                            "outline" => {
                                run.text_outline(size * 0.3, color);
                            }
                            "reflection" => {
                                run.reflection();
                            }
    ...
    #[tool(
        description = "Add a run with a theme color (accent1-6, dk1, dk2, lt1, lt2, hlink). Colors auto-update when theme changes."
    )]
    pub async fn add_theme_colored_text(
        &self,
        Parameters(input): Parameters<ThemeColorRunInput>,
    ) -> String {
        with_doc!(
            self.store,
            input.document_handle,
    ...
    #[tool(
        description = "Apply banded rows and header styling to a table. Makes tables look professional with alternating colors."
    )]
    pub async fn style_table_banded(
        &self,
        Parameters(input): Parameters<BandedTableInput>,
    ) -> String {
        with_doc!(
            self.store,
            input.document_handle,
    ...
    #[tool(
        description = "Enable line numbering in the document margin (for legal/academic documents)."
    )]
    pub async fn set_line_numbering(
        &self,
        Parameters(input): Parameters<LineNumberingInput>,
    ) -> String {
        with_doc!(
            self.store,
            input.document_handle,
            |doc: &mut zavora_docx::Document| {
                doc.set_line_numbering(
                    input.count_by.unwrap_or(1),
                    input.restart.as_deref().unwrap_or("continuous"),
                );
                serde_json::json!({"set": true}).to_string()
            }
        )
    ...
    #[tool(
        description = "Create a list with custom numbering: Roman numerals, letters, or custom bullet characters (★, →, ◆)."
    )]
    ...
    #[tool(
        description = "Set the document theme (fonts and colors). Controls the look of all theme-colored text."
    )]
    pub async fn set_theme(&self, Parameters(input): Parameters<ThemeInput>) -> String {
        with_doc!(
            ...
            |doc: &mut zavora_docx::Document| {
                ...
                let a = |i: usize| {
                    accents
                        .and_then(|v| v.get(i).map(|s| s.as_str()))
...
mod parse_tests {
    ...
    fn parses_text_array_and_object_fields() {
        ...
        assert_eq!(
            f[1]["item_keys"],
            serde_json::json!(["description", "qty", "price", "amount?"])
        );
    ...
    fn comma_split_respects_nested_brackets() {
        ...
        assert_eq!(
            f[1]["item_keys"],
            serde_json::json!(["title", "dates", "bullets[]"])
        );
    }
}
